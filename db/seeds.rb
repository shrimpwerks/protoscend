# This file should contain all the record creation needed to seed the database with its default values.
# The data can then be loaded with the rake db:seed (or created alongside the db with db:setup).

SmallGroup.create(
  group_name: 'Test Group',
  hidden: 0
)

Announcement.create(
  announcement: 'hey look everybody, i\'m announcing this!!1'
)

User.create(
  fname: 'Ty',
  lname: 'Skelton',
  email: 'test.email@mail.com',
  password: 'encrypt-this',
  user_level: 1,
  small_groups_id: 1,
  hidden: 0,
  auth_token: '1'
)

User.create(
  fname: 'Waka Flocka',
  lname: 'Flame',
  email: 'waka.f.flame@mail.com',
  password: 'encrypt-this',
  user_level: 2,
  small_groups_id: 1,
  hidden: 0,
  auth_token: '2'
)

User.create(
  fname: 'Chris',
  lname: 'Sharma',
  email: 'chris.sharma@mail.com',
  password: 'encrypt-this',
  user_level: 1,
  small_groups_id: 1,
  hidden: 0,
  auth_token: '3'
)

User.create(
  fname: 'Red',
  lname: 'Teletubby',
  email: 'red.teletubby@mail.com',
  password: 'encrypt-this',
  user_level: 3,
  small_groups_id: 1,
  hidden: 0,
  auth_token: '4'
)

User.create(
  fname: 'Steve',
  lname: 'Jobs',
  email: 'steve.jobs@me.com',
  password: 'encrypt-this',
  user_level: 1,
  small_groups_id: 1,
  hidden: 0,
  auth_token: '5'
)

User.create(
  fname: 'Optimus',
  lname: 'Prime',
  email: 'autobots.rollout@spark.com',
  password: 'encrypt-this',
  user_level: 5,
  small_groups_id: 1,
  hidden: 0,
  auth_token: '6'
)

User.create(
  fname: 'Bear',
  lname: 'Grylls',
  email: 'peejoke@mail.com',
  password: 'encrypt-this',
  user_level: 1,
  small_groups_id: 1,
  hidden: 0,
  auth_token: '7'
)

User.create(
  fname: 'Obi Wan',
  lname: 'Kenobi',
  email: 'bigballer69@aol.com',
  password: 'encrypt-this',
  user_level: 3,
  small_groups_id: 1,
  hidden: 0,
  auth_token: '8'
)

User.create(
  fname: 'Donald',
  lname: 'Trump',
  email: 'racist@gop.com',
  password: 'encrypt-this',
  user_level: 1,
  small_groups_id: 1,
  hidden: 0,
  auth_token: '9'
)

User.create(
  fname: 'Barbara',
  lname: 'Streisand',
  email: 'b.streisand@live.com',
  password: 'encrypt-this',
  user_level: 2,
  small_groups_id: 1,
  hidden: 0,
  auth_token: '10'
)

AssignedRoute.create(
  users_id: 1,
  gym: 'Dixon',
  grade: '5.10+',
  completed: 0,
  hidden: 0
)

AvailableWall.create(
  location: 'Dixon',
  wall_id: 0
)

AvailableWall.create(
  location: 'Dixon',
  wall_id: 1
)

Route.create(
  route_name: 'Metalocalypse',
  user_id: 1,
  available_walls_id: 1,
  location: 'Dixon',
  tape_color: 'Blue',
  grade: '5.9+',
  route_set_date: '2015-01-01',
  expiration_date: '2015-04-01'
)

Route.create(
  route_name: 'Best Route',
  user_id: 2,
  available_walls_id: 2,
  location: 'Dixon',
  tape_color: 'Red',
  grade: '5.10',
  route_set_date: '2015-05-05',
  expiration_date: '2015-08-05'
)

Rating.create(
  routes_id: 1,
  users_id: 1,
  rating: 5
)

Rating.create(
  routes_id: 2,
  users_id: 2,
  rating: 4
)

Rating.create(
  routes_id: 1,
  users_id: 4,
  rating: 3
)

MaintenanceRequest.create(
  routes_id: 1,
  users_id: 1,
  reason: 'spinner in the middle',
  resolved: 0,
  priority: 1
)

Comment.create(
  routes_id: 1,
  users_id: 1,
  comment: 'excellent route, brochacho'
)

SupportTicket.create(
  users_id: 1,
  ticket: 'i had a bad experience with the site and I choose to not elaborate.'
)
